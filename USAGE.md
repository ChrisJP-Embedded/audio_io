...
uPlot waveform, time-window zoom controls, and a buffered FFT chart. With the
default float audio format, `0 dBFS` is full scale and quieter signals are
negative values. The default `--block-words 256` keeps GUI updates responsive
while leaving enough samples for stable RMS readings.
